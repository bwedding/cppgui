    ...
    try
    {
        ...
        switch (dbType)
        {
            ...
            case DatabaseType::PostgreSQL:
            {
                ...
                LOGI << "Opening PostgreSQL connection";
    ...
    *success = FALSE;
    ...
    try
    {
        ...
        if (pgIt != m_postgresConnections.end())
        {
            ...
            *success = TRUE;
            LOGI << "Closed PostgreSQL connection";
            return S_OK;
        }

        LOGW << "No active connection found to close";
    ...
    *success = FALSE;
    ...
    try
    {
        ...
        // Check if there's already an active transaction for this connection
        {
            ...
            if (txnIt != m_activeTransactions.end() && txnIt->second)
            {
                LOGW << "Transaction already in progress for this connection";
                return S_OK; // Return success but with *success = FALSE
            }
        }
        ...
        switch (dbType)
        {
            case DatabaseType::SQLite:
            {
                ...
                sqlite3* db = nullptr;
                {
                    ...
                    if (connIt == m_sqliteConnections.end())
                    {
                        LOGE << "No open SQLite connection found";
                        return S_OK; // Return success but with *success = FALSE
                    }
                ...
                *success = TRUE;
                LOGI << "SQLite transaction started";
                break;
            }
            ...
            case DatabaseType::PostgreSQL:
            {
                ...
                *success = TRUE;
                LOGI << "PostgreSQL transaction started";
                break;
            }
...
STDMETHODIMP NativeDatabaseAccess::CommitTransaction(BSTR connectionString, BOOL * success)
{
    ...
    *success = FALSE;
    ...
    try
    {
        ...
        if (!hasActiveTxn)
        {
            LOGW << "No active transaction to commit";
            return S_OK; // Return success but with *success = FALSE
        }
        ...
        switch (dbType)
        {
            case DatabaseType::SQLite:
            {
                ...
                *success = TRUE;
                LOGI << "SQLite transaction committed";
                break;
            }
            ...
            case DatabaseType::PostgreSQL:
            {
                ...
                *success = TRUE;
                LOGI << "PostgreSQL transaction committed";
                break;
            }
...
{
    LOGD << "RollbackTransaction called";
    ...
    *success = FALSE;
    ...
    try
    {
        ...
        if (!hasActiveTxn)
        {
            LOGW << "No active transaction to rollback";
            return S_OK; // Return success but with *success = FALSE
        }
        ...
        switch (dbType)
        {
            case DatabaseType::SQLite:
            {
                ...
                *success = TRUE;
                LOGI << "SQLite transaction rolled back";
                break;
            }
            ...
            case DatabaseType::PostgreSQL:
            {
                ...
                *success = TRUE;
                LOGI << "PostgreSQL transaction rolled back";
                break;
            }
...
{
    LOGD << "CreateParameterizedQuery called";
...
{
    LOGD << "ExecuteParameterizedQuery called";
...
{
    LOGD << "ExecuteParameterizedNonQuery called";
