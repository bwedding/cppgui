...
struct impl : api
{
    ...
    bool read(const uint32_t format, const sv path, const sv data) noexcept override
    {
        ...
        if (format == JSON)
        {
            success = detail::seek_impl([&](auto&& val)
            {
        ...
        else
        {
            ...
            {
                pe = glz::read<opts{.format = BEVE}>(val, data);
            }, user, path);
        }
    ...
    bool write(const uint32_t format, const sv path, std::string& data) noexcept override
    {
        ...
        if (format == JSON)
        {
            ...
            {
                std::ignore = glz::write_json(val, data);
            }, user, path);
        }
        else
        {
            return detail::seek_impl([&](auto&& val)
            {
                std::ignore = glz::write_beve(val, data);
            }, user, path);
        }
    }
...
{
    return std::shared_ptr<impl<T>> {new impl<T>{}, [](impl<T>* ptr)
    {
        delete ptr;
    }
                                    };
