...
template <class T>
concept erasable = requires(T v)
{
    v.erase(v.cbegin(), v.cend());
};

template <class T>
concept has_size = requires(T v)
{
    v.size();
};
...
template <class T>
concept has_append = requires(T t, typename T::const_iterator it)
{
    t.append(it, it);
};

template <class T>
concept has_assign = requires(T t, const typename T::value_type* v, typename T::size_type sz)
{
    t.assign(v, sz);
};
